...
export default function Interview() {
    ...
    const { data: questionsData, isFetching: isStartingInterview, error: startError } = useStartInterview(interviewId, !questionsFromNav.length);
    ...
    useEffect(() => {
        if (!isInterviewStarted && questionsData.length > 0 && !isStartingInterview) {
            console.log("Starting interview", { timestamp: new Date().toISOString() });
            handleStartInterview();
        }
    }, [isInterviewStarted, questionsData.length, isStartingInterview, handleStartInterview]);

    // Show toast notification when a screenshot is captured
    useEffect(() => {
        if (lastCapturedScreenshot) {
            console.log("Screenshot captured", { count: screenshots.length, timestamp: new Date().toISOString() });
    ...
    useEffect(() => {
        if (startError) {
            console.log("Start interview error", { error: startError.message, timestamp: new Date().toISOString() });
                ...
                <div className="flex-1 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-md w-full">
                        ...
                        <p className="text-gray-600 dark:text-gray-300 mb-4">
                            No questions available for this interview. Please go back and try again.
                        </p>
                        <Button
