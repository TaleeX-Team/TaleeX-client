...
const UsersPage = () => {
    ...
    const [selectedUser, setSelectedUser] = useState(null); // Added missing state declaration
    ...
    const confirmDelete = () => {
        ...
        deleteUserMutation.mutate(
            {userId: selectedUser._id, password: deletePassword},
            {
                onSuccess: () => {
    ...
    const handleEditUser = (user) => {
        ...
        toast.info(`Edit user: ${user.firstName} ${user.lastName}`, {
            description: `User ID: ${user._id}`,
        });
    };

    const handleUserUpdated = () => {
        refetch(); // Uncommented to ensure list refreshes after update
        setProfileViewOpen(false); // Close the profile view if open
        setEditDialogOpen(false); // Close the edit dialog if open
    };
    ...
    return (
        <>
            <Card className="border border-border shadow-lg transition-all duration-200 hover:shadow-md" ref={cardRef}>
                <CardHeader className="pb-4" ref={headerRef}>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <CardTitle
                                className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">Users</CardTitle>
                        ...
                        <TooltipProvider>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    ...
                                    {/*    <span*/}
                                    {/*        className="absolute inset-0 bg-primary/10 transform scale-0 group-hover:scale-100 transition-transform duration-300 rounded-md"></span>*/}
                ...
                <CardContent className="space-y-6">
                    {/* Filters */}
                    <div
                        className="bg-muted/50 p-4 rounded-lg border border-border transition-all duration-300 hover:border-border/80">
                        <div className="flex flex-col md:flex-row gap-4">
                            <div className="relative flex-1 group">
                                <Search
                                    className="absolute left-3 top-3 h-4 w-4 text-muted-foreground group-focus-within:text-primary transition-colors duration-200"/>
                                <Input
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                <Select value={roleFilter} onValueChange={setRoleFilter}>
                                    <SelectTrigger
                                        className="w-full sm:w-40 bg-background border-border transition-all duration-200">
                                ...
                                <Select value={statusFilter} onValueChange={setStatusFilter}>
                                    ...
                                    <SelectContent className="bg-background border-border">
                                        <SelectGroup>
                                            <SelectLabel className="text-muted-foreground">Filter by
                                                status</SelectLabel>
                    ...
                    {/* Users table */}
                    <div
                        className="border border-border rounded-lg overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md">
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm" ref={tableRef}>
                                ...
                                <tr className="bg-muted/70 border-b border-border">
                                    ...
                                    <th className="px-6 py-4 text-left font-medium text-muted-foreground">Last Active
                                    </th>
                                    ...
                                    Array(5).fill(0).map((_, index) => (
                                        <tr key={index} className="border-b border-border">
                                            ...
                                            <td className="px-6 py-4"><Skeleton
                                                className="h-6 w-16 rounded-full bg-muted"/></td>
                                            <td className="px-6 py-4"><Skeleton className="h-4 w-24 bg-muted"/></td>
                                            <td className="px-6 py-4 text-right"><Skeleton
                                                className="h-8 w-8 rounded-full bg-muted ml-auto"/></td>
                                        </tr>
                                    ))
                                ) : filteredUsers.length > 0 ? (
                                    filteredUsers.map((user) => (
                                        <tr key={user._id}
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <Avatar
                                                        className="h-10 w-10 border border-border ring-2 ring-background transition-all duration-300 hover:ring-primary/20">
                                                        <AvatarImage src={user.image || user.socialLoginAvatar}
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center">
                                                    <span
                                                        className="font-medium text-foreground">{user.role || "User"}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <Badge
                                                    variant="outline"
                                                    className={`${getStatusStyles(getUserStatus(user))} font-medium capitalize border-none transition-all duration-200`}
                                                >
                                                    {getUserStatus(user)}
                                                </Badge>
                                            </td>
                                                    ...
                                                    <DropdownMenuContent align="end"
                                                        ...
                                                        >
                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                 className="h-4 w-4 mr-2" width="24" height="24"
                                        ...
                                        <td colSpan={5} className="px-6 py-10 text-center text-muted-foreground">
                                            <div className="flex flex-col items-center justify-center space-y-4">
                                                <div className="relative">
                                                    ...
                                                    <span
                                                        className="absolute top-0 right-0 h-3 w-3 rounded-full bg-primary/30 animate-ping"></span>
                                                </div>
                                                <div>
                                                    <p className="text-lg font-medium text-foreground">No users
                                                        found</p>
                                                    <p className="text-sm">Try adjusting your search or filter
                                                        criteria</p>
                                                </div>
                                                <Button
                                                    variant="outline"
            ...
            {selectedUser && (
                <UserProfileDialog
                    userId={selectedUser._id}  // Pass userId instead of user object
            ...
            {selectedUser && (
                <EditUserDialog
                    user={selectedUser}  // This component expects the full user object
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <DialogFooter className="sm:justify-between">
                        ...
                        <Button
                            ...
                            disabled={!deletePassword || deleteUserMutation.isPending}
                        >
