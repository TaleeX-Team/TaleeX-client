...
const AnalyticsPage = () => {
  ...
  const handleDownloadPdf = async () => {
    ...
    for (const config of chartReportConfigs) {
      if (yPos > 250) {
        // Check for page break before starting a new chart section
        doc.addPage();
        yPos = 15;
      }
      ...
      if (currentChartData && currentChartData.length > 0) {
        if (yPos > 265 && !chartImageGenerated) {
          // If no image and table is long, ensure space
          doc.addPage();
          yPos = 15;
        }
        autoTable(doc, {
  ...
  const chartReportConfigs = [
    {
      ...
      dataFormatter: formatDailyApplicationsForChart,
      refIndex: 0,
      columns: ["Date", "Applications"],
      bodyAccessor: (item) => [item.name, item.applications],
    },
    {
      title: "Applications by Stage",
      dataFormatter: formatApplicationsByStageForChart,
      refIndex: 1,
      columns: ["Stage", "Count"],
      bodyAccessor: (item) => [item.name, item.value],
    },
    {
      title: "Interviews by State",
      dataFormatter: formatInterviewsByStateForChart,
      refIndex: 2,
      columns: ["State", "Count"],
      bodyAccessor: (item) => [item.name, item.value],
    },
    {
      ...
      dataFormatter: formatConversionFunnelForChart,
      refIndex: 3,
      columns: ["Stage", "Count", "Rate (%)"],
      bodyAccessor: (item) => [item.name, item.count, item.rate],
    },
    {
      title: "Top 5 Applied Jobs",
      dataFormatter: formatTopAppliedJobsForChart,
      refIndex: 4,
