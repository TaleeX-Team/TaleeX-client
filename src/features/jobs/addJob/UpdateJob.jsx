...
const formSchema = z.object({
    ...
    (data) => {
        if (data.salary?.min && data.salary?.max) {
            return Number.parseFloat(data.salary.min) <= Number.parseFloat(data.salary.max)
        }
        return true
    },
    {
...
export default function UpdateJob({ jobId, trigger }) {
    ...
    const { data: companies = [], isLoading: isCompaniesLoading } = useNonRejectedCompanies()
    ...
    const loadJobData = useCallback(async () => {
        ...
        try {
            ...
            const companyId = typeof data.company === "object" ? data.company?.id || data.company?._id : data.company
    ...
    const onSubmit = async (values) => {
        try {
            ...
            const transformedValues = {
                ...
                tags: values.tags || [],
                salary: {
                    min: values.salary?.min ? Number.parseFloat(values.salary.min) : undefined,
                    max: values.salary?.max ? Number.parseFloat(values.salary.max) : undefined,
                    currency: values.salary?.currency || undefined,
                },
                hiringTarget: values.hiringTarget ? Number.parseInt(values.hiringTarget, 10) : undefined,
            }

            // Remove undefined values for cleaner API payload
            if (!transformedValues.salary.min && !transformedValues.salary.max && !transformedValues.salary.currency) {
                delete transformedValues.salary;
            }

            await updateJobMutation.mutateAsync(transformedValues)
    ...
    const filteredTags = COMMON_TAGS.filter(
        (tag) => !tags.includes(tag) && tag.toLowerCase().includes(currentTag.toLowerCase())
    )
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[800px] md:max-w-[900px] p-0 bg-background">
                <DialogHeader className="px-6 pt-6 pb-2">
                    <DialogTitle className="text-2xl font-semibold">Update Job Posting</DialogTitle>
                            ...
                            <Form {...form}>
                                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                    <TabsContent value="basics" className="space-y-4 py-2">
                                                    ...
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        ...
                                                        <SelectContent>
                                                            {isCompaniesLoading ? (
                                                                ...
                                                                companies.map((comp) => (
                                                                    <SelectItem key={comp.id || comp._id} value={comp.id || comp._id}>
                                                                        {comp.name}
                                                                    </SelectItem>
                                                                ))
                                                            ) : (
                                                                <div className="p-2 text-sm">No companies available</div>
                                                            )}
                                                        </SelectContent>
                                                    </Select>
                                                    <FormMessage />
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel className="text-sm font-medium">Workplace Type *</FormLabel>
                                                        <Select onValueChange={field.onChange} value={field.value}>
                                                            <FormControl>
                                                                <SelectTrigger>
                                    ...
                                    <TabsContent value="additional" className="space-y-6 py-2">
                                        ...
                                        <div>
                                            <h3 className="text-sm font-medium mb-3">Salary Information</h3>
                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                                <FormField
                                                    control={form.control}
                                                    name="salary.min"
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel className="text-sm font-medium">Min Salary</FormLabel>
                                                            <FormControl>
                                                ...
                                                <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel className="text-sm font-medium">Max Salary</FormLabel>
                                                            <FormControl>
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            ...
                                                            <Select onValueChange={field.onChange} value={field.value}>
                                                                ...
                                                                <SelectContent>
                                                                    {["USD", "EUR", "SAR", "AED", "KWD", "EGP"].map((curr) => (
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel className="text-sm font-medium">Hiring Target</FormLabel>
                                                    <FormControl>
                                    ...
                                    <DialogFooter className="px-6 py-4 flex justify-between sm:justify-end gap-2 border-t">
                                        ...
                                        <Button
                                            type="submit"
                                            disabled={isLoading || isCompaniesLoading || !form.formState.isValid}
                                        >
                                            {isLoading ? (
                                                <>
                                                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
