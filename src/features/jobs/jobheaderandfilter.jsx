...
// Job filter options based on API schema
...
const COMMON_TAGS = ["JavaScript", "React", "Node.js", "Python", "Java", "DevOps", "AWS", "UI/UX", "Product Management", "Marketing"]
...
export default function JobsHeaderWithFilters({ onFilterChange }) {
  ...
  useEffect(() => {
    ...
    if (selectedTags.length > 0)
      newActiveFilters.push({ type: "tags", value: `${selectedTags.length} tags` });
    if (salaryMin)
                ...
                {activeFilters.map((filter) => (
                    <Badge key={filter.type} variant="secondary" className="flex items-center gap-1">
                      {filter.type === 'title' ? 'Title' :
                          filter.type === 'requirements' ? 'Requirements' :
                              filter.type === 'company' ? 'Company' :
                                  filter.type === 'workPlaceType' ? 'Workplace' :
                                      filter.type === 'jobType' ? 'Job Type' :
                                          filter.type === 'experienceLevel' ? 'Experience' :
                                              filter.type === 'status' ? 'Status' :
                                                  filter.type === 'tags' ? 'Tags' :
                                                      filter.type === 'salaryMin' ? 'Min Salary' :
                                                          filter.type === 'salaryMax' ? 'Max Salary' :
                                                              filter.type === 'createdAtFrom' ? 'From Date' : 'To Date'}: {filter.value}
                      <button
                          type="button"
