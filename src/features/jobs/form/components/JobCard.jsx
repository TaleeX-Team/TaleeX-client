...
import { Briefcase, Building2, Clock, DollarSign, MapPin, User, ExternalLink, Edit } from "lucide-react"
...
export default function JobCard({ job }) {
  const { _id, title, company, workPlaceType, jobType, experienceLevel, applicationLink, status, salary, openTime } =
      job
      ...
      <Card className="overflow-hidden hover:shadow-lg dark:hover:shadow-slate-900/30 transition-all duration-300 flex flex-col h-full border-slate-200 dark:border-slate-800 group">
        <CardHeader className="pb-2 pt-5 px-5 space-y-2">
          <div className="flex justify-between items-start gap-4">
            ...
            <Badge className={`${getStatusColor(status)} capitalize text-xs font-medium px-2.5 py-1`}>{status}</Badge>
          </div>
        </CardHeader>
        ...
        <CardContent className="px-5 py-3 flex-grow">
          <div className="grid grid-cols-2 gap-y-3 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-700 dark:text-slate-300 capitalize">{workPlaceType}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-700 dark:text-slate-300 capitalize">{jobType.replace("-", " ")}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-700 dark:text-slate-300">{experienceLevel}</span>
            </div>
            ...
            <UpdateJob
                ...
                trigger={
                  <Button
                      variant="outline"
                      size="sm"
                      className="gap-1.5 font-medium border-slate-200 dark:border-slate-700 hover:bg-amber-50 hover:text-amber-600 hover:border-amber-200 dark:hover:bg-amber-900/20 dark:hover:text-amber-400 dark:hover:border-amber-800"
                  >
