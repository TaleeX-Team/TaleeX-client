      ...
      <div className="w-full max-w-6xl space-y-6 py-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-8">
            ...
            <Section title="Candidate Summary" icon={<User className="text-primary h-5 w-5" />}>
              <p className="text-muted-foreground">{feedback.summary}</p>
            </Section>
            <Section title="Requirements Analysis" icon={<FileText className="text-primary h-5 w-5" />}>
              <RequirementsList requirements={feedback.requirementMatch} />
            </Section>
            <Section title="Seniority Assessment" icon={<Briefcase className="text-primary h-5 w-5" />}>
    ...
    <div>
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="w-full md:w-2/3">
          ...
          <Progress value={score} className="h-3" indicatorClassName={getScoreBackground(score)} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      {requirements.map((req, idx) => {
        ...
        return (
          <div key={idx} className="rounded-lg border p-4 hover:bg-muted/5 transition-colors">
    ...
    <div className="flex items-start gap-3">
      ...
      <div>
        <h4 className="font-medium mb-1">
          {seniorityFit.match === "Yes" ? "Appropriate Seniority Level" : "Seniority Mismatch"}
        </h4>
...
function RecommendationCard({ recommendation }) {
  ...
  return (
    <Card className={isRecommended ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}>
      <CardHeader>
        <div className="flex items-center gap-2">
          <ThumbsUp className={`h-5 w-5 ${isRecommended ? "text-green-600" : "text-red-600"}`} />
          <CardTitle className="text-lg">Final Recommendation</CardTitle>
        </div>
      </CardHeader>
      <CardContent>
        <div className="flex items-center gap-3 mb-3">
          <h3 className={`text-xl font-semibold ${isRecommended ? "text-green-700" : "text-red-700"}`}>
            {isRecommended ? "Recommended for Hire" : "Not Recommended"}
          </h3>
          <Badge className={isRecommended ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"}>
        ...
        <div className="mt-3">
          ...
          {!isRecommended && (
            <div className="mt-4 p-3 bg-white rounded-md border border-red-100">
              <h4 className="text-sm font-medium mb-1">Alternative Suggestions</h4>
              <p className="text-sm text-muted-foreground">
                Consider this candidate for junior developer positions or internship opportunities where they can grow their React skills.
              </p>
            </div>
          )}
        </div>
