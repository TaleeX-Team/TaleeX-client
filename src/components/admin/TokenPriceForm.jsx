...
import { PoundSterlingIcon as CurrencyPound, DollarSign, Euro, PoundSterling } from "lucide-react"

const formSchema = z.object({
    tokenValue: z.coerce.number().positive({
...
export default function TokenPriceForm({
    ...
    const form = useForm({
        ...
        defaultValues: {
            tokenValue: tokenPrice?.tokenValue || (typeof tokenPrice === "number" ? tokenPrice : 0.1),
    ...
    useEffect(() => {
        if (tokenPrice) {
            const value = tokenPrice?.tokenValue || (typeof tokenPrice === "number" ? tokenPrice : 0.1)
            ...
            <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6">
                ...
                <FormField
                    ...
                    <span className="text-gray-500 sm:text-sm">
                      {isEGP ? "£E" : currentCurrency === "USD" ? "$" : currentCurrency === "EUR" ? "€" : "£"}
                    </span>
                                    </div>
                                    <Input type="number" step="0.01" min="0.01" placeholder="0.10" className="pl-10" {...field} />
                                </div>
                            </FormControl>
                            <FormDescription>This is the amount in {currentCurrency} that a single token will cost.</FormDescription>
                            <FormMessage />
                        </FormItem>
                    )}
                />
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <Select onValueChange={field.onChange} value={field.value}>
                                ...
                                <SelectContent>
                                    {supportedCurrencies.includes("EGP") && <SelectItem value="EGP">EGP - Egyptian Pound</SelectItem>}
                                    {supportedCurrencies.includes("USD") && <SelectItem value="USD">USD - US Dollar</SelectItem>}
                                    {supportedCurrencies.includes("EUR") && <SelectItem value="EUR">EUR - Euro</SelectItem>}
                                    {supportedCurrencies.includes("GBP") && <SelectItem value="GBP">GBP - British Pound</SelectItem>}
                                </SelectContent>
                            </Select>
                            <FormMessage />
                        </FormItem>
                ...
                <Button type="submit" className="w-full" disabled={isSubmitting}>
                    {isSubmitting ? "Updating..." : `Update ${currentCurrency} Token Value`}
                </Button>
            </form>
        </Form>
    )
