...
import { Calendar, Mail, MapPin, Phone, Globe, Briefcase, Shield, Clock } from "lucide-react";
import {
    Dialog,
    DialogContent,
    DialogDescription,
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-start gap-4 pb-2 border-b border-border">
                            <Avatar className="h-20 w-20 border border-border ring-2 ring-background">
                                <AvatarImage src={user.image || user.socialLoginAvatar} alt={getUserFullName(user)} />
                                <AvatarFallback className="bg-muted text-primary text-lg font-medium">
                                    {getUserInitials(user)}
                                </AvatarFallback>
                            </Avatar>
                            <div className="space-y-1 flex-1">
                                <h3 className="text-xl font-semibold text-foreground">{getUserFullName(user)}</h3>
                                <div className="flex items-center gap-2">
                                    <Badge
                                        variant="outline"
                                        className={`${getStatusStyles(getUserStatus(user))} font-medium capitalize border-none`}
                                    >
                                        {getUserStatus(user)}
                                    </Badge>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {user.lastLoginDate && (
                                <div className="flex items-center gap-2 text-foreground">
                                    ...
                                    <span>Last active: {new Date(user.lastLoginDate).toLocaleString()}</span>
                                </div>
                            )}
                        </div>
