      ...
      interviewTimerRef.current = setInterval(() => {
        setTimeRemaining((prev) => {
          ...
          if (newTime <= 0 && canSpeak && !conclusionDetected) {
            ...
            if (debugMode)
              console.log(
                "Interview duration reached, ending with conclusion phrase",
                {
                  timestamp: new Date().toISOString(),
                }
              );
      ...
      if (screenshots.length < MAX_SCREENSHOTS) {
        const intervalId = setInterval(() => {
          ...
          if (screenshots.length >= MAX_SCREENSHOTS) {
            ...
            if (debugMode)
              console.log(
                "Screenshot interval cleared after reaching max screenshots",
                {
...
- Ensure every question transition (except the first) is announced with "Moving to question X".`,
      ...
      const call = await vapiClientRef.current.start(
        VAPI_ASSISTANT_ID,
        interviewer
      );
      setCallId(call.id);
  ...
  const toggleAudio = useCallback(() => {
    ...
    try {
      ...
      setError(
        `Warning: Could not toggle microphone: ${
